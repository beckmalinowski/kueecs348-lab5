...
int main() {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        char range[18];
        // snprintf sets fstring %s-%s (January-June, February-July etc.) to 18 char
        // (whitespace if it doesn't fill it all)
    ...
    // temporary array will be destroyed in sorting process
    float temp[12];
    for (int i = 0; i < 12; i++) {
        temp[i] = sales[i];
    }
...
int min(float sales[]) {
    ...
    int low = 0; // since high is an index, we can set it to zero instead of inf
    for (int i = 0; i < 12; i++) {
...
int max(float sales[]) {
    ...
    int high = 0; // since high is an index, we can set it to zero instead of -inf
    for (int i = 0; i < 12; i++) {
